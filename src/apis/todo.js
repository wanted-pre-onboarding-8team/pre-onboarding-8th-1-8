...
export const updateTodo = formData => client.put(`/todos/${formData.id}`, {
  todo: formData.todo, isCompleted: formData.isCompleted,
}, { headers: { Authorization: `Bearer ${formData.accessToken}` } });

export const deleteTodo = formData => client.delete(`/todos/${formData.id}`, { headers: { Authorization: `Bearer ${formData.accessToken}` } });
